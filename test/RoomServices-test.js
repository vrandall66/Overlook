...
chai.spy.on(domUpdates, ["displayRoomServiceCharges", "displayAllRoomServiceOrders"], () => {});
...
describe("RoomServices", () => {
  ...
  it("should find all daily rooms that ordered room service", () => {
    ...
    expect(roomServiced.findAllRoomService("2019/07/28")).to.eql([
      { "date": "2019/07/28", "food": "Licensed Soft Sandwich", "totalCost": 20.09, "userID": 91 },
      { "date": "2019/07/28", "food": "Handmade Cotton Sandwich", "totalCost": 24.74, "userID": 46 },
      { "date": "2019/07/28", "food": "Refined Wooden Sandwich", "totalCost": 20.52, "userID": 53 }
    ]);
  });
  ...
  it("should find all items ordered on a given day", () => {
    ...
    expect(domUpdates.displayAllRoomServiceOrders).to.have.been.called(12);
  })

  it("should display information to the DOM", () => {
    expect(roomServiced.displayToDom).to.be.a("function");
    expect(domUpdates.displayRoomServiceCharges).to.have.been.called(5);
  })
});
